...
public:
    ...
    BlockEpilogue(Arch::Resource<ArchTag> const &resource, Params const &params = Params{}) : params(params)
    {
        ...
        for (int32_t i = 0; i < UB_STAGES; ++i) {
            ubCHList[i] = resource.ubBuf.template GetBufferByByte<ElementC>(ubOffset);  // 高4位用
            ubOffset += max_len * sizeof(ElementC);
            ubCLList[i] = resource.ubBuf.template GetBufferByByte<ElementC>(ubOffset);  // 低4位用
            ubOffset += max_len * sizeof(ElementC);
        ...
        ubFp32 = resource.ubBuf.template GetBufferByByte<float>(ubOffset);  // 高4为转fp32用
        ubOffset += max_len * sizeof(float);
        ubFp32L = resource.ubBuf.template GetBufferByByte<float>(ubOffset);  // 低4位转fp32用
        ubOffset += max_len * sizeof(float);
                    ...
                    int32_t groupIdx, int32_t preSrcExpertSum, AscendC::GlobalTensor<int32_t> preSumBeforeRank)
    {
        ...
        auto gmWeighAux = weightAux[gmweighAuxOffset];
#ifdef W4A8_DEBUG
...
*/
        ...
        for (int32_t row = 0; row < actualBlockShape.m() / 2; ++row) {
            ...
            AscendC::SetFlag<AscendC::HardEvent::S_V>(0);  //  S
