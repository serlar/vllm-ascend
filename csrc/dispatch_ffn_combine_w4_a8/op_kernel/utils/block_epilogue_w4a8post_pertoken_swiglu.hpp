...
public:
    ...
    void operator()(AscendC::GlobalTensor<ElementC> const &gmC, MatrixCoord const &shapeC,
                    ...
                    AscendC::GlobalTensor<float> const &weightAux, AscendC::GlobalTensor<ElementD> const &gmD,
                    ...
                    uint32_t EP, AscendC::GlobalTensor<float> const &gmGMM1, int32_t rank,
                    uint32_t epilogueCoreNum = 40, Callback &&callback = Callback{})
    {
        ...
        for (uint32_t loopIdx = loopStartIdx; loopIdx < loopStartIdx + tasksForIdx; ++loopIdx) {
            ...
            if (loopIdx < loopStartIdx + UB_STAGES)  // 第一次初始化辅助矩阵
            {
            ...
            AscendC::SetFlag<AscendC::HardEvent::S_V>(0);  //  S是标量操作，保证320行
            ...
            PipeBarrier<PIPE_V>();
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_V>(eventxHighMTE3VList[ubListId]);  // eventid需要调整
            ...
            if (LAST_LEN_VK > 0) {
                And(xLowHalfTensor[LEN_VK * LEN_128].template ReinterpretCast<int16_t>(),
                    ubD[LEN_VK * LEN_128 * TWO].template ReinterpretCast<int16_t>(), xLowI16Tensor, LAST_LEN_VK, 1,
                    {1, 1, 1, 8, 8, 0});
            }
            PipeBarrier<PIPE_V>();
